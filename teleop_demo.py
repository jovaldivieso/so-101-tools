...
def main():
    ...
    cameras = {
        "wrist_cam": OpenCVCameraConfig(
            index_or_path=Path(CAM_INNOMAKER), fps=30, width=640, height=480
        ),
        "context_cam": OpenCVCameraConfig(
            index_or_path=Path(CAM_WED), fps=30, width=640, height=480
        ),
    }

    LEADER_PATH = "/dev/serial/by-id/usb-1a86_USB_Single_Serial_5AB9068767-if00"
    FOLLOWER_PATH = "/dev/serial/by-id/usb-1a86_USB_Single_Serial_5AB9068718-if00"

    robot_cfg = SOFollowerRobotConfig(
        id="follower", port=FOLLOWER_PATH, cameras=cameras
    )

    teleop_cfg = SOLeaderTeleopConfig(id="leader", port=LEADER_PATH)
